 ...
 * @return NULL on error or ObjData otherwise.  Note: this must be freed with
 * gf3d_obj_free
 */
...
/**
 * @brief re-organize the vertices into faceVertices for use with the rendering
 * pipeline
 ...
 * @param rotation apply this rotation to the vertices and normals
 * @return NULL if the input objs are missing or have not been re-organized yet,
 * a new third OBJ containing all the geometry of the first two.
 * @note this does NOT free the input OBJs.
 * @note gf3d_obj_load_reorg must have been called on both prior to calling this
 * one
 */
ObjData *
...
/**
 * @brief perform a collision test between the edge and an obj.  Searches
 * through each triangle for collision, returning the FIRST triangle with
 * collision (so convex hulls might return the wrong side)
 * @param obj the object to test
 * @param offset if the model has moved, rotated, scaled etc.  this is applied
 * before the test
