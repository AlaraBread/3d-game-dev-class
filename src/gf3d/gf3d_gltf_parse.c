...
char *gfc_base64_encode(const void *input, size_t inputLength, size_t *newSize) {
	...
	for(x = 0; x < inputLength; x += 3) {
		...
		if((x + 1) < inputLength) {
			if(resultIndex >= resultSize) {
				free(result);
				return NULL; /* indicate failure: buffer too
						small */
			}
		...
		/*
		 * if we have all three bytes available, then their encoding is
		 * spread out over four characters
		 */
		if((x + 2) < inputLength) {
			if(resultIndex >= resultSize) {
				free(result);
	...
	/*
	 * create and add padding that is required if we did not have a multiple
	 * of 3 number of characters available
	 */
	if(padCount > 0) {
