 ...
 * @param config path to a json formatted config file containing device
 * information for prioritizing which device to use
...
/**
 * @brief get the physical device that best matches the configured priorities
 * for devices
 * @return VK_NULL_HANDLE if uninitialized or no device set or the physical
 * device handle
 */
 ...
 * @return VK_NULL_HANDLE if no matches or the physical device handle in
 * question
 */
 ...
 * @return NULL if it has not been chosen yet, or an issue arose while choosing,
 * or the device info otherwise
 */
GF3D_Device *gf3d_device_get_chosen_gpu_info();

/**
 * @brief get the creation info needed to create a logical device based on what
 * has been loaded and configured so far
 * @param enableValidationLayers if true, this will turn on validation layers.
 * @return an empty (all zeros) logical device create info, or a configured one
 * otherwise
 */
VkDeviceCreateInfo
