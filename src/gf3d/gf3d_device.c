...
void gf3d_device_manager_close() {
	if(gf3d_device_manager.device != VK_NULL_HANDLE) { vkDestroyDevice(gf3d_device_manager.device, NULL); }
	sj_free(gf3d_device_manager.config);
	if(gf3d_device_manager.device_list) { gfc_list_foreach(gf3d_device_manager.device_list, free); }
...
GF3D_Device *gf3d_device_get_info(VkPhysicalDevice device) {
	...
	if(device_config) {
		...
		if(sj_object_get_value(device_config, "geometryShader")) {
			if(device_info->deviceFeatures.geometryShader) { device_info->score++; }
		}
	...
	if(__DEBUG) {
		...
		slog(
			"Dedicated GPU: %i",
			(device_info->deviceProperties.deviceType == VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU) ? 1 : 0
		);
...
GF3D_Device *gf3d_device_get_chosen_gpu_info() { return gf3d_device_manager.chosen_gpu; }
...
VkPhysicalDevice gf3d_devices_get_device_by_name(const char *name) {
	...
	for(i = 0; i < c; i++) {
		device_info = gfc_list_get_nth(gf3d_device_manager.device_list, i);
		if(!device_info) continue;
		if(strcmp(name, device_info->deviceProperties.deviceName) == 0) { return device_info->device; }
	}
	return VK_NULL_HANDLE;
}

VkPhysicalDevice gf3d_devices_get_best_device() {
	if(gf3d_device_manager.bestDevice == -1) { gf3d_device_manager_determine_best(); }
