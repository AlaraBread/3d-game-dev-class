...
typedef struct {
	...
	VkSurfaceKHR renderSurface; /**<vulkan surface target for the
				   screen/window owned by graphics*/
} GF3D_DeviceManager;
...
VkDeviceCreateInfo
	gf3d_device_get_logical_device_info(Bool enableValidationLayers);
...
void gf3d_device_manager_close() {
	...
	if(gf3d_device_manager.devices) { free(gf3d_device_manager.devices); }
	memset(&gf3d_device_manager, 0, sizeof(GF3D_DeviceManager));
	if(__DEBUG) slog("gf2d_devices manager closed");
}

void gf3d_device_manager_init(
	const char *config, VkInstance instance, VkSurfaceKHR renderSurface
) {
	SJson *deviceConfig;
	...
	if(!gf3d_device_manager.chosen_gpu) { return; }

	deviceConfig = sj_object_get_value(gf3d_device_manager.config, "devices");
	if(deviceConfig) {
		sj_get_bool_value(
			sj_object_get_value(
				gf3d_device_manager.config, "enable_validation"
			),
			&enable_validation
		);
	}
	...
	gf3d_vqueues_init(
		gf3d_device_manager.chosen_gpu->device,
		gf3d_device_manager.renderSurface
	);
...
GF3D_Device *gf3d_device_get_info(VkPhysicalDevice device) {
	...
	if(device_config) {
		if(sj_object_get_value(device_config, "discrete")) {
			if(device_info->deviceProperties.deviceType ==
	...
	if(__DEBUG) {
		...
		slog(
			"Dedicated GPU: %i", (device_info->deviceProperties.deviceType ==
								  VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU)
									 ? 1
									 : 0
		);
		...
		slog(
			"supports Geometry Shader: %i",
			device_info->deviceFeatures.geometryShader
		);
	}
...
int gf3d_devices_enumerate() {
	...
	gf3d_device_manager.devices =
		(VkPhysicalDevice *)malloc(sizeof(VkPhysicalDevice) * count);
	vkEnumeratePhysicalDevices(
		gf3d_device_manager.instance, &count, gf3d_device_manager.devices
	);
...
void gf3d_device_manager_determine_best() {
	...
	if(__DEBUG) {
		if(device_best) {
			slog(
				"best CPU chosen: %s", device_best->deviceProperties.deviceName
			);
		} else {
...
VkPhysicalDevice gf3d_devices_get_best_device() {
	...
	if(gf3d_device_manager.bestDevice == -1) { return VK_NULL_HANDLE; }
	return gf3d_device_manager.devices[gf3d_device_manager.bestDevice];
}

VkDevice gf3d_device_get() { return gf3d_device_manager.device; }

VkDevice gf3d_device_create_logic_device(Bool enableValidationLayers) {
	VkPhysicalDevice gpu;
	VkDeviceCreateInfo createInfo =
		gf3d_device_get_logical_device_info(enableValidationLayers);
	...
	if(vkCreateDevice(gpu, &createInfo, NULL, &gf3d_device_manager.device) !=
	   VK_SUCCESS) {
...
VkDeviceCreateInfo
	...
	queueCreateInfo =
		(VkDeviceQueueCreateInfo *)gf3d_vqueues_get_queue_create_info(&count);
	if(!queueCreateInfo) {
	...
	createInfo.pEnabledFeatures =
		&gf3d_device_manager.chosen_gpu->deviceFeatures;

	createInfo.ppEnabledExtensionNames =
		gf3d_extensions_get_device_enabled_names(&count);
