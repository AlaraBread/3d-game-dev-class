...
void gf3d_model_update_uniform_buffer(
	Model *model, UniformBuffer *ubo, GFC_Matrix4F modelMat, GFC_Vector4D colorMod, GFC_Vector4D ambientLight
);
...
void gf3d_model_manager_close() {
	...
	if(gf3d_model.model_list) { free(gf3d_model.model_list); }
...
Model *gf3d_model_new() {
	...
	for(i = 0; i < gf3d_model.max_models; i++) {
		if(!gf3d_model.model_list[i].refCount) {
			...
			gfc_matrix4f_identity(gf3d_model.model_list[i].matrix);
...
Model *gf3d_model_copy(Model *in) {
	...
	if(in->material) {
		out->material = in->material; // if set, use this material when
									  // sending draw calls
		out->material->_refCount++;
	}

	if(in->texture) {
		out->texture = in->texture; // if set, use this material when
									// sending draw calls
		out->texture->_refcount++;
	}
	if(in->normalMap) {
		out->normalMap = in->normalMap; // if set, use this material
										// when sending draw calls
...
Model *gf3d_model_load_from_config(SJson *json, const char *filename) {
	...
	if(modelFile) {
		...
		if(sj_object_get_value(json, "obj_list")) {
			...
			if(!mesh) {
				slog(
					"failed to parse mesh data from obj file "
					"%s",
					modelFile
				);
				gf3d_model_free(model);
	...
	if(item) { sj_value_as_matrix4_vectors(item, model->matrix); }
	...
	if(materialFile) {
		...
		if(item) { model->material = gf3d_material_parse_js(item, filename); }
	}

	textureFile = sj_get_string_value(sj_object_get_value(json, "normalMap"));
	if(textureFile) { model->normalMap = gf3d_texture_load(textureFile); }

	textureFile = sj_get_string_value(sj_object_get_value(json, "texture"));
	if(textureFile) { model->texture = gf3d_texture_load(textureFile); }
	return model;
}
...
void gf3d_model_free(Model *model) {
	...
	model->refCount--;
	if(model->refCount <= 0) { gf3d_model_delete(model); }
}
...
void gf3d_model_mat_scale(ModelMat *mat, GFC_Vector3D scale) {
	if(!mat) return;
	gfc_vector3df_multiply(mat->scale, gfc_vector3d_to_float(scale));
}
...
void gf3d_model_mat_reset(ModelMat *mat) {
	...
	gfc_matrix4f_identity(mat->mat);
	mat->scale = gfc_vector3df(1, 1, 1);
}

void gf3d_model_mat_extract_vectors(ModelMat *mat) {
	if(!mat) return;
	gfc_matrix4f_to_vectors(mat->mat, &mat->position, &mat->rotation, &mat->scale);
}

void gf3d_model_mat_set_matrix(ModelMat *mat) {
	if(!mat) return;
	gfc_matrix4f_from_vectors(mat->mat, mat->position, mat->rotation, mat->scale);
}
...
ModelMat *gf3d_model_mat_new() {
	...
	gfc_matrix4f_identity(modelMat->mat);
	gfc_vector3d_set(modelMat->scale, 1, 1, 1);
	return modelMat;
}

MeshUBO gf3d_model_get_sky_ubo(GFC_Matrix4F modelMat, GFC_Vector4DF colorMod) {
