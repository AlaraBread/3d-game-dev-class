...
typedef struct {
	...
	VkDescriptorSet *descriptorSet; // pointer to the descriptorSet we will
									// use for this draw call
 ...
 * @param vertFile the filename of the vertex shader to use (expects spir-v byte
 * code)
 * @param fragFile the filename of the fragment shader to use (expects spir-v
 * byte code)
 ...
 * @param descriptorCount the number of concurrent descriptSets to be supported
 * per command, ie: how many models you want to support for a draw call  This
 * should be based on maximum number of supported entities or graphic assets
 * @param vertexInputDescription the vertex input description to use
 * @param vertextInputAttributeDescriptions list of how the attributes are
 * described
 ...
 * @param indexType VK_INDEX_TYPE_UINT16, VK_INDEX_TYPE_UINT32, or
 ...
 * byte code)
 * @param extent the viewport dimensions for this pipeline
 * @param descriptorCount the number of concurrent descriptSets to be suppert
 * per command, ie: how many models you want to support for a draw call  This
 * should be based on maximum number of supported entities or graphic assets
 * @returns NULL on error (see logs) or a pointer to a pipeline
 */
...
/**
 * @brief get a descriptor set to be used for the pipeline.  Provide the swap
 * chain rendering frame.
...
/**
 * @brief submit the render calls to the pipeline for this frame.  Called after
 * reset_frame and all draw calls
 * @param pipe for the pipe in question
 */
void gf3d_pipeline_submit_commands(Pipeline *pipe);

/**
 * @brief submit the commands for ALL pipelines in the order in which they were
 * created
 * @note order might be messed up if any were destroyed and recreated during the
 * life of the program
 */
