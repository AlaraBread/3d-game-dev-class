...
void gf3d_vqueues_choose_present_family() {
	...
	for(i = 0; i < gf3d_vqueues.queue_family_count; i++) {
		vkGetPhysicalDeviceSurfaceSupportKHR(gf3d_vqueues.physical_device, i, gf3d_vqueues.surface, &supported);
...
void gf3d_vqueues_init(VkPhysicalDevice device, VkSurfaceKHR surface) {
	...
	gf3d_vqueues.queue_list[VQ_Graphics].queue_priority =
		1 / 3.0; // EVEN priorities now, but this should be configurable
	...
	vkGetPhysicalDeviceQueueFamilyProperties(device, &gf3d_vqueues.queue_family_count, NULL);
	...
	gf3d_vqueues.queue_family_properties =
		(VkQueueFamilyProperties *)gfc_allocate_array(sizeof(VkQueueFamilyProperties), gf3d_vqueues.queue_family_count);

	vkGetPhysicalDeviceQueueFamilyProperties(
		device, &gf3d_vqueues.queue_family_count, gf3d_vqueues.queue_family_properties
	);
	...
	for(i = 0; i < gf3d_vqueues.queue_family_count; i++) {
		if(__DEBUG) {
			...
			slog(
				"queue min image transfer granularity %iw %ih %id",
	...
	if(gf3d_vqueues.queue_list[VQ_Graphics].queue_family != -1) { gf3d_vqueues.work_queue_count++; }
	if((gf3d_vqueues.queue_list[VQ_Present].queue_family != -1) &&
	   (gf3d_vqueues.queue_list[VQ_Present].queue_family != gf3d_vqueues.queue_list[VQ_Graphics].queue_family)) {
	...
	if(!gf3d_vqueues.work_queue_count) {
		...
		gf3d_vqueues.queue_create_info = (VkDeviceQueueCreateInfo *)gfc_allocate_array(
			sizeof(VkDeviceQueueCreateInfo), gf3d_vqueues.work_queue_count
		);
		i = 0;
...
void gf3d_vqueues_setup_device_queues(VkDevice device) {
	if(gf3d_vqueues.queue_list[VQ_Graphics].queue_family != -1) {
		vkGetDeviceQueue(
			device, gf3d_vqueues.queue_list[VQ_Graphics].queue_family, 0, &gf3d_vqueues.queue_list[VQ_Graphics].queue
		);
	}
	if(gf3d_vqueues.queue_list[VQ_Present].queue_family != -1) {
		vkGetDeviceQueue(
			device, gf3d_vqueues.queue_list[VQ_Present].queue_family, 0, &gf3d_vqueues.queue_list[VQ_Present].queue
		);
	}
	if(gf3d_vqueues.queue_list[VQ_Transfer].queue_family != -1) {
		vkGetDeviceQueue(
			device, gf3d_vqueues.queue_list[VQ_Transfer].queue_family, 0, &gf3d_vqueues.queue_list[VQ_Transfer].queue
		);
	}
}

void gf3d_vqueues_close() {
	if(gf3d_vqueues.queue_create_info) { free(gf3d_vqueues.queue_create_info); }
	if(gf3d_vqueues.queue_family_properties) { free(gf3d_vqueues.queue_family_properties); }
	memset(&gf3d_vqueues, 0, sizeof(vQueues));
	if(__DEBUG) slog("vqueues closed");
}

Sint32 gf3d_vqueues_get_graphics_queue_family() { return gf3d_vqueues.queue_list[VQ_Graphics].queue_family; }

Sint32 gf3d_vqueues_get_present_queue_family() { return gf3d_vqueues.queue_list[VQ_Present].queue_family; }

Sint32 gf3d_vqueues_get_transfer_queue_family() { return gf3d_vqueues.queue_list[VQ_Transfer].queue_family; }

VkQueue gf3d_vqueues_get_graphics_queue() { return gf3d_vqueues.queue_list[VQ_Graphics].queue; }

VkQueue gf3d_vqueues_get_present_queue() { return gf3d_vqueues.queue_list[VQ_Present].queue; }

VkQueue gf3d_vqueues_get_transfer_queue() { return gf3d_vqueues.queue_list[VQ_Transfer].queue; }
/*eol@eof*/
