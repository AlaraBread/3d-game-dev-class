...
typedef struct {
	VkPhysicalDevice
		physical_device;  /**<this device was used to set up the queues*/
	VkSurfaceKHR surface; /**<presentation is to this surface*/
	Uint32 queue_family_count;
	VkQueueFamilyProperties *queue_family_properties;
	Uint32 work_queue_count;
	VkDeviceQueueCreateInfo
		*queue_create_info; /**<used when the logical device is created*/
...
void gf3d_vqueues_choose_graphics_family() {
	...
	for(i = 0; i < gf3d_vqueues.queue_family_count; i++) {
		if(gf3d_vqueues.queue_family_properties[i].queueFlags &
...
void gf3d_vqueues_choose_present_family() {
	...
	for(i = 0; i < gf3d_vqueues.queue_family_count; i++) {
		vkGetPhysicalDeviceSurfaceSupportKHR(
			gf3d_vqueues.physical_device, i, gf3d_vqueues.surface, &supported
		);
...
void gf3d_vqueues_init(VkPhysicalDevice device, VkSurfaceKHR surface) {
	...
	vkGetPhysicalDeviceQueueFamilyProperties(
		device, &gf3d_vqueues.queue_family_count, NULL
	);
	...
	gf3d_vqueues.queue_family_properties =
		(VkQueueFamilyProperties *)gfc_allocate_array(
			sizeof(VkQueueFamilyProperties), gf3d_vqueues.queue_family_count
		);

	vkGetPhysicalDeviceQueueFamilyProperties(
		device, &gf3d_vqueues.queue_family_count,
		gf3d_vqueues.queue_family_properties
	);
	...
	for(i = 0; i < gf3d_vqueues.queue_family_count; i++) {
		if(__DEBUG) {
			...
			slog(
				"queue flag bits %i",
				gf3d_vqueues.queue_family_properties[i].queueFlags
			);
			slog(
				"queue count %i",
				gf3d_vqueues.queue_family_properties[i].queueCount
			);
			slog(
				"queue timestamp valid bits %i",
				gf3d_vqueues.queue_family_properties[i].timestampValidBits
			);
			slog(
				"queue min image transfer granularity %iw %ih %id",
				gf3d_vqueues.queue_family_properties[i]
	...
	if(__DEBUG) {
		slog(
			"using queue family %i for graphics commands",
			gf3d_vqueues.queue_list[VQ_Graphics].queue_family
		);
		slog(
			"using queue family %i for rendering pipeline",
			gf3d_vqueues.queue_list[VQ_Present].queue_family
		);
		slog(
			"using queue family %i for transfer pipeline",
			gf3d_vqueues.queue_list[VQ_Transfer].queue_family
		);
	}
	...
	if(!gf3d_vqueues.work_queue_count) {
		...
		gf3d_vqueues.queue_create_info =
			(VkDeviceQueueCreateInfo *)gfc_allocate_array(
				sizeof(VkDeviceQueueCreateInfo), gf3d_vqueues.work_queue_count
			);
		i = 0;
		if(gf3d_vqueues.queue_list[VQ_Graphics].queue_family != -1) {
			gf3d_vqueues.queue_create_info[i++] =
				gf3d_vqueues_get_graphics_queue_info();
		}
		if((gf3d_vqueues.queue_list[VQ_Present].queue_family != -1) &&
		   (gf3d_vqueues.queue_list[VQ_Present].queue_family !=
			gf3d_vqueues.queue_list[VQ_Graphics].queue_family)) {
			gf3d_vqueues.queue_create_info[i++] =
				gf3d_vqueues_get_present_queue_info();
		}
	}
...
VkDeviceQueueCreateInfo gf3d_vqueues_get_present_queue_info() {
	...
	queueCreateInfo.queueCount = 1;
	queueCreateInfo.pQueuePriorities =
		&gf3d_vqueues.queue_list[VQ_Present].queue_priority;
	return queueCreateInfo;
}
...
VkDeviceQueueCreateInfo gf3d_vqueues_get_transfer_queue_info() {
	...
	queueCreateInfo.queueCount = 1;
	queueCreateInfo.pQueuePriorities =
		&gf3d_vqueues.queue_list[VQ_Transfer].queue_priority;
	return queueCreateInfo;
}
...
void gf3d_vqueues_setup_device_queues(VkDevice device) {
	if(gf3d_vqueues.queue_list[VQ_Graphics].queue_family != -1) {
		vkGetDeviceQueue(
			device, gf3d_vqueues.queue_list[VQ_Graphics].queue_family, 0,
			&gf3d_vqueues.queue_list[VQ_Graphics].queue
		);
	}
	if(gf3d_vqueues.queue_list[VQ_Present].queue_family != -1) {
		vkGetDeviceQueue(
			device, gf3d_vqueues.queue_list[VQ_Present].queue_family, 0,
			&gf3d_vqueues.queue_list[VQ_Present].queue
		);
	}
	if(gf3d_vqueues.queue_list[VQ_Transfer].queue_family != -1) {
		vkGetDeviceQueue(
			device, gf3d_vqueues.queue_list[VQ_Transfer].queue_family, 0,
			&gf3d_vqueues.queue_list[VQ_Transfer].queue
		);
	}
}
