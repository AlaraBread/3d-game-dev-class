...
/**
 * @brief the structure as it appears in the shaders.  Order must match what is
 * in the vertex and fragment shaders
 */
typedef struct {
	GFC_Vector4D ambient;  // how much ambient light affects this material
	GFC_Vector4D diffuse;  // how much diffuse light affects this material -
...
typedef struct {
	...
	GFC_Color diffuse;		// how much diffuse light affects this material -
							// primary influcen for material color
 ...
 * @return a zero material if no material provided, or one set to the provided
 * one.
 */
 ...
 * @note there may be duplicate names.  You may need to use
 * gf3d_material_get_by_file_name to specify which file it came from
 * @param name the name of the material to find
 * @return NULL if not found, or a pointer to the first material with that name
 * otherwise
 */
 ...
 * @note this doesn't return anything, but all the materials are loaded into
 * memory
 * @note they can be retreived with calls to gf3d_material_get_by_file_name or
 * gf3d_material_get_by_name
