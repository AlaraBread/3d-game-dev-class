...
/**
 * @brief initializes the mesh system / configures internal data about mesh
 * based rendering
 * @param mesh_max the maximum allowed simultaneous meshes supported at once.
 * Must be > 0
 * @note keep in mind that many models will be comprised of multiple sub meshes.
 * So this number may need to be very large
 */
 ...
 * @note this free's the intermediate data loaded from the obj file, no longer
 ...
 * @note: this merges them at the vertex and face buffer level, making the
 * primitives of A inclusive of the meshes from B.
 * @note: this only applies for the N primitives, which is the smaller of the
 * two lists
 * @param meshA the mesh to gain geometry
 * @param meshB the mesh to provide new geometry, this is unmodified after the
 * process
 ...
 * @return the factor to scale a mesh so that it fits exactly within the size
 * provided.
 * @note: likely you want to uniformly scale based on the SMALLEST of the
 * dimensions
 */
 ...
 * @note the primitive must have the objData set and it must have be organizes
 * in buffer order
 */
...
/**
 * @brief given a model matrix and basic color, build the meshUBO needed to
 * render a model
