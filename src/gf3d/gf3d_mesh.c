...
void gf3d_mesh_init(Uint32 mesh_max) {
	Uint32 count = 0;
	if(!mesh_max) {
		slog("failed to initialize mesh system: cannot allocate 0 "
			 "mesh_max");
		return;
	}
...
Pipeline *gf3d_mesh_get_pipeline() { return gf3d_mesh.pipe; }

Pipeline *gf3d_mesh_get_sky_pipeline() { return gf3d_mesh.sky_pipe; }
...
void gf3d_mesh_queue_render(
	...
	for(i = 0; i < c; i++) {
		...
		gf3d_pipeline_queue_render(
			...
			primitive->faceBuffer, uboData, texture
		);
	}
}
...
Mesh *gf3d_mesh_load_obj(const char *filename) {
	...
	memcpy(&mesh->bounds, &obj->bounds, sizeof(GFC_Box)); // TODO: this
														  // isn't right
