...
/**
 * @brief init Vulkan / SDL, setup device and initialize infrastructure for 3d
 * graphics
 ...
 * @note: THIS SHOULD ONLY BE CALLED BETWEEN CALLS TO
 * gf3d_vgraphics_render_start() and gf3d_vgraphics_render_end()
 ...
 * @note this does the same operations that are better handled in shaders, so it
 * sparingly
 ...
 * sparingly
 * @param position the position in 3D space input
 * @return the same position as it maps to the screen with the z component being
 * the screen depth
 */
 ...
 * @param surface a pointer to the SDL_Surface pointer that contains the image
 * data to convert
 * @return NULL on failure, or a new SDL surface of the same image, but in the
 * supported format.
 * @note this will clear the data of the original surface if it is successful
 * automatically.
 */
