...
/**
 * @brief set the movement step for camera movement with
...
/**
 * @brief explicitely set the camera positon, holding all other parameters the
 * same
...
/**
 * @brief get the rotation angles of the camera that can be used with
 * gfc_vector3d_angle_vectors()
 * @return the angle, in radians of the camera
 */
GFC_Vector3D gf3d_camera_get_angles();

/**
 * @brief get the gfc_vectors for forward, right, and up based on the camera
 * view angles
 * @param forward   [output] if provided, it will be set with the normal
 * gfc_vector in the direction the camera is looking
 * @param right     [output] if provided, it will be set with the normal
 * gfc_vector to the right of where the camera is looking
 ...
 * @note it may still need to be enabled and you will need to call
 * gf3d_camera_controls_update
 */
