...
typedef struct {
	...
	VkBuffer faceBuffer;			 /**<memory handle for the face buffer (always two
							faces)*/
...
SpriteUBO gf2d_sprite_get_uniform_buffer(
	Sprite *sprite, GFC_Vector2D position, GFC_Vector2D scale, GFC_Vector3D rotation, GFC_Color color,
...
void gf2d_sprite_manager_close() {
	...
	if(gf2d_sprite.sprite_list) { free(gf2d_sprite.sprite_list); }
	if(gf2d_sprite.faceBuffer != VK_NULL_HANDLE) { vkDestroyBuffer(gf2d_sprite.device, gf2d_sprite.faceBuffer, NULL); }
...
void gf2d_sprite_manager_init(Uint32 max_sprites) {
	...
	gf3d_buffer_create(
		...
		VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, &gf2d_sprite.faceBuffer, &gf2d_sprite.faceBufferMemory
	);
	...
	gf2d_sprite.pipe = gf3d_pipeline_create_from_config(
		gf3d_vgraphics_get_default_logical_device(), "assets/config/overlay_pipeline.cfg",
		gf3d_vgraphics_get_view_extent(), max_sprites, gf2d_sprite_get_bind_description(),
		gf2d_sprite_get_attribute_descriptions(NULL), count, sizeof(SpriteUBO), VK_INDEX_TYPE_UINT16
	);
...
void gf3d_sprite_submit_pipe_commands() { gf3d_pipeline_submit_commands(gf2d_sprite.pipe); }
...
Sprite *gf2d_sprite_get_by_filename(const char *filename) {
	...
	for(i = 0; i < gf2d_sprite.max_sprites; i++) {
		...
		if(gfc_line_cmp(gf2d_sprite.sprite_list[i].filename, filename) == 0) { return &gf2d_sprite.sprite_list[i]; }
	}
	return NULL;
}
...
Sprite *gf2d_sprite_load_image(const char *filename) { return gf2d_sprite_load(filename, 0, 0, 1); }
...
void gf2d_sprite_delete(Sprite *sprite) {
	...
	if(sprite->buffer != VK_NULL_HANDLE) { vkDestroyBuffer(gf2d_sprite.device, sprite->buffer, NULL); }
	if(sprite->bufferMemory != VK_NULL_HANDLE) { vkFreeMemory(gf2d_sprite.device, sprite->bufferMemory, NULL); }
...
void gf2d_sprite_draw_image(Sprite *sprite, GFC_Vector2D position) {
	gf2d_sprite_draw(sprite, position, NULL, NULL, NULL, NULL, NULL, NULL, 0);
}

void gf2d_sprite_draw_simple(Sprite *sprite, GFC_Vector2D position, Uint32 frame) {
	gf2d_sprite_draw(sprite, position, NULL, NULL, NULL, NULL, NULL, NULL, frame);
}
...
void gf2d_sprite_draw(
	...
	if(center) { gfc_vector2d_copy(drawRotation, (*center)); }
	...
	spriteUBO = gf2d_sprite_get_uniform_buffer(
		sprite, position, drawScale, drawRotation, drawGFC_ColorShift, drawClip, drawFlip, frame
	);

	gf3d_pipeline_queue_render(
		gf2d_sprite.pipe, sprite->buffer,
		6, // its a single quad
		gf2d_sprite.faceBuffer, &spriteUBO, &sprite->texture->textureImageView, &sprite->texture->textureSampler,
		VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_NULL_HANDLE, VK_NULL_HANDLE, VK_IMAGE_LAYOUT_UNDEFINED
	);
}
...
void gf2d_sprite_create_vertex_buffer(Sprite *sprite) {
	...
	gf3d_buffer_create(
		...
		VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, &sprite->buffer, &sprite->bufferMemory
	);
...
void gf2d_sprite_draw_to_surface(
	Sprite *sprite, GFC_Vector2D position, GFC_Vector2D *scale, GFC_Vector2D *center, Uint32 frame, SDL_Surface *surface
) {
	...
	if(scale) { gfc_vector2d_copy(scaleFactor, (*scale)); }
	if(center) { gfc_vector2d_copy(scaleOffset, (*center)); }
	fpl = (sprite->framesPerLine) ? sprite->framesPerLine : 1;
	gfc_rect_set(
		cell, frame % fpl * sprite->frameWidth, frame / fpl * sprite->frameHeight, sprite->frameWidth,
		sprite->frameHeight
	);
	gfc_rect_set(
		target, position.x - (scaleFactor.x * scaleOffset.x), position.y - (scaleFactor.y * scaleOffset.y),
		sprite->frameWidth * scaleFactor.x, sprite->frameHeight * scaleFactor.y
	);
	SDL_BlitScaled(sprite->surface, &cell, surface, &target);
}
...
SpriteUBO gf2d_sprite_get_uniform_buffer(
	...
	if(flip.x) { scale.x = fabs(scale.x) * -1; }
...
Pipeline *gf2d_sprite_get_pipeline() { return gf2d_sprite.pipe; }
