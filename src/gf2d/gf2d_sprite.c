...
typedef struct {
	...
	VkVertexInputAttributeDescription
		attributeDescriptions[SPRITE_ATTRIBUTE_COUNT];
...
SpriteUBO gf2d_sprite_get_uniform_buffer(
	...
	GFC_Vector3D rotation, GFC_Color color, GFC_Vector4D clip,
	GFC_Vector2D flip, Uint32 frame
);
...
void gf2d_sprite_manager_init(Uint32 max_sprites) {
	...
	gf2d_sprite.sprite_list =
		(Sprite *)gfc_allocate_array(sizeof(Sprite), max_sprites);
	...
	vkMapMemory(
		gf2d_sprite.device, stagingBufferMemory, 0, bufferSize, 0, &data
	);
	...
	gf3d_buffer_create(
		...
		VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, &gf2d_sprite.faceBuffer,
		&gf2d_sprite.faceBufferMemory
	);
	...
	gf2d_sprite.pipe = gf3d_pipeline_create_from_config(
		...
		"assets/config/overlay_pipeline.cfg", gf3d_vgraphics_get_view_extent(),
		max_sprites, gf2d_sprite_get_bind_description(),
		gf2d_sprite_get_attribute_descriptions(NULL), count, sizeof(SpriteUBO),
...
Sprite *gf2d_sprite_from_surface(
	...
	sprite->heightPercent =
		sprite->frameHeight / (float)sprite->texture->height;
	if(frames_per_line)
...
void gf2d_sprite_draw_full(
	...
	gf2d_sprite_draw(
		sprite, position, &scale, &center, &rotation, &flip, &colorShift, &clip,
		frame
	);
}
...
void gf2d_sprite_draw_simple(
	...
	gf2d_sprite_draw(
		sprite, position, NULL, NULL, NULL, NULL, NULL, NULL, frame
	);
}
...
void gf2d_sprite_draw(
	...
	spriteUBO = gf2d_sprite_get_uniform_buffer(
		sprite, position, drawScale, drawRotation, drawGFC_ColorShift, drawClip,
		drawFlip, frame
	);
...
void gf2d_sprite_create_vertex_buffer(Sprite *sprite) {
	...
	gf3d_buffer_create(
		...
		VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, &sprite->buffer,
		&sprite->bufferMemory
	);
...
void gf2d_sprite_draw_to_surface(
	Sprite *sprite, GFC_Vector2D position, GFC_Vector2D *scale,
	GFC_Vector2D *center, Uint32 frame, SDL_Surface *surface
) {
	...
	gfc_rect_set(
		cell, frame % fpl * sprite->frameWidth,
		frame / fpl * sprite->frameHeight, sprite->frameWidth,
		sprite->frameHeight
	);
	gfc_rect_set(
		target, position.x - (scaleFactor.x * scaleOffset.x),
...
SpriteUBO gf2d_sprite_get_uniform_buffer(
	...
	SpriteUBO spriteUBO = {0};
	spriteUBO.size =
		gfc_vector2d(sprite->texture->width, sprite->texture->height);
...
VkVertexInputAttributeDescription *
	...
	gf2d_sprite.attributeDescriptions[0].offset =
		offsetof(SpriteVertex, vertex);
