...
void gf2d_fonts_load_json(const char *filename) {
	...
	for(i = 0; i < count; i++) {
		...
		if(str) { gfc_line_cpy(font_manager.font_list[i].filename, str); }
...
GFC_Rect gf2d_font_get_text_wrap_bounds(
	...
	int space = 0;
	int lindex = 0;
	if((thetext == NULL) || (thetext[0] == '\0')) { return r; }
	if(font == NULL) {
...
void gf2d_font_draw_text_wrap(
	...
	do {
		...
		if(w > block.w) /*see if we have gone over*/
		{
			...
			if(block.h != 0) {
				if((drawheight + (h * row)) > (block.y + block.h)) { break; }
			}
			gfc_block_sprintf(temptextline, "%s", word); /*add a word*/
		}
	} while(1);
