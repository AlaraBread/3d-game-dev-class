...
void gf2d_draw_image_new(Sprite *sprite, GFC_Shape shape, Uint8 filled) {
	DrawImage *image;
	if(!sprite) return;
...
DrawImage *gf2d_draw_image_get(GFC_Shape shape, Uint8 filled) {
	DrawImage *image;
	int i, c;
	c = gfc_list_get_count(draw_manager.draw_images);
	for(i = 0; i < c; i++) {
...
void gf2d_draw_circle_filled(GFC_Vector2D center, int radius, GFC_Color color) {
	...
	if(!pointArray) {
		slog("gf2d_draw_circle: failed to allocate points for circle "
			 "drawing");
		return;
	}
