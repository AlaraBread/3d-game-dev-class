 ...
 */
GFC_Vector3DF gfc_unprojectf(GFC_Vector3DF in,GFC_Matrix4F view, GFC_Matrix4F proj,GFC_Vector2DF viewport)
{
    GFC_Vector3DF out = {0,0,0};
    GFC_Matrix4F InvProj = {0},InvView = {0};
    GFC_Vector4DF tmp,obj,eye;
    ...
    gfc_matrix4f_invert(InvProj,proj);
    gfc_matrix4f_invert(InvView,view);
...
void _gfc_matrix4f_slog(const char *filename,Uint32 line,GFC_Matrix4F mat)
{
...
void _gfc_matrix3f_slog(const char *filename,Uint32 line,GFC_Matrix3F mat)
{
    _slog(filename,line,"%f,%f,%f,%f",mat[0][0],mat[0][1],mat[0][2],mat[0][3]);
    _slog(filename,line,"%f,%f,%f,%f",mat[1][0],mat[1][1],mat[1][2],mat[1][3]);
    _slog(filename,line,"%f,%f,%f,%f",mat[2][0],mat[2][1],mat[2][2],mat[2][3]);
...
void _gfc_matrix2f_slog(const char *filename,Uint32 line,GFC_Matrix2F mat)
{
    _slog(filename,line,"%f,%f,%f,%f",mat[0][0],mat[0][1],mat[0][2],mat[0][3]);
    _slog(filename,line,"%f,%f,%f,%f",mat[1][0],mat[1][1],mat[1][2],mat[1][3]);
}

void gfc_matrix4f_to_vectors(
    GFC_Matrix4F in,
    GFC_Vector3DF *translation,
    ...
    GFC_Vector3DF *scale)
{
    ...
    if (rotation)
    {
        ...
        {
            rotation->x = -atan2(in[0][2], in[2][3]);
            rotation->z = 0;
            rotation->y = 0;
...
void gfc_matrix4f_from_vectors(
    GFC_Matrix4F out,
    GFC_Vector3DF translation,
    GFC_Vector3DF rotation,
    GFC_Vector3DF scale)
{
    gfc_matrix4f_identity(out);
    
    gfc_matrix4f_scale(out,out,scale);
    gfc_matrix4f_rotate_by_vector(out,out,rotation);
    gfc_matrix4f_translate(out,out,translation);
}

void gfc_matrix4f_to_matrix16f(float m16[16],GFC_Matrix4F m4)
{
    m16[0]  = m4[0][0];
    m16[1]  = m4[0][1];
    m16[2]  = m4[0][2];
...
void gfc_matrix16f_to_matrix4f(GFC_Matrix4F m4,float m16[16])
{
    m4[0][0] = m16[0]  ;
    m4[0][1] = m16[1]  ;
    m4[0][2] = m16[2]  ;
...
Uint8 gfc_matrix16f_invert(float m[16], float invOut[16])
{
    float inv[16], det;
    int i;
...
Uint8 gfc_matrix4f_invert(GFC_Matrix4F mOut, GFC_Matrix4F mIn)
{
    float m[16];
    gfc_matrix4f_to_matrix16f(m,mIn);
    
    if (!gfc_matrix16f_invert(m,m))return 0;
    gfc_matrix16f_to_matrix4f(mOut,m);
    return 1;
}


void gfc_matrix2f_copy(GFC_Matrix2F d,GFC_Matrix2F s)
{
    if ((!d)||(!s))return;
    if (d == s)return;
    memcpy(d,s,sizeof(GFC_Matrix2F));
}

void gfc_matrix3f_copy(GFC_Matrix3F d,GFC_Matrix3F s)
{
    if ((!d)||(!s))return;
    if (d == s)return;
    memcpy(d,s,sizeof(GFC_Matrix3F));
}

void gfc_matrix4f_copy(GFC_Matrix4F d,GFC_Matrix4F s)
{
    if ((!d)||(!s))return;
    if (d == s)return;
    memcpy(d,s,sizeof(GFC_Matrix4F));
}


void gfc_matrix4f_multiply_scalar(GFC_Matrix4F out,GFC_Matrix4F m1,float s)
{
    GFC_Matrix4F temp;
    temp[0][0] = s*m1[0][0];
    temp[0][1] = s*m1[0][1];
    ...
    temp[3][2] = s*m1[3][2];
    temp[3][3] = s*m1[3][3];
    gfc_matrix4f_copy(out,temp);
}

void gfc_matrix3f_multiply_scalar(GFC_Matrix3F out,GFC_Matrix3F m1,float s)
{
    GFC_Matrix3F temp;
    temp[0][0] = s*m1[0][0];
    temp[0][1] = s*m1[0][1];
    ...
    temp[2][1] = s*m1[2][1];
    temp[2][2] = s*m1[2][2];
    gfc_matrix3f_copy(out,temp);
}

void gfc_matrix2f_multiply_scalar(GFC_Matrix2F out,GFC_Matrix2F m1,float s)
{
    GFC_Matrix2F temp;
    temp[0][0] = s*m1[0][0];
    temp[0][1] = s*m1[0][1];
    ...
    temp[1][0] = s*m1[1][0];
    temp[1][1] = s*m1[1][1];
    gfc_matrix2f_copy(out,temp);
}

void gfc_matrix4f_multiply(
    GFC_Matrix4F out,
    GFC_Matrix4F m2,
    GFC_Matrix4F m1)
{
    GFC_Matrix4F out1;//just in case its the same matrix
    ...
    gfc_matrix4f_copy(out,out1);
}

void gfc_matrix3f_multiply(
    GFC_Matrix3F out,
    GFC_Matrix3F m2,
    GFC_Matrix3F m1)
{
    ...
    gfc_matrix3f_copy(out,out1);
}

void gfc_matrix2f_multiply(
    GFC_Matrix2F out,
    GFC_Matrix2F m2,
    GFC_Matrix2F m1)
{
    ...
    gfc_matrix2f_copy(out,out1);
}


void gfc_matrix4f_v_multiply(GFC_Vector4DF *out,GFC_Vector4DF vec,GFC_Matrix4F mat)
{
  float ox,oy,oz,ow;
  if (!out)return;
...
void gfc_matrix3f_v_multiply(GFC_Vector3DF *out,GFC_Vector3DF vec,GFC_Matrix3F mat)
{
  float ox,oy,oz;
  if (!out)return;
...
void gfc_matrix2f_v_multiply(GFC_Vector2DF *out,GFC_Vector2DF vec,GFC_Matrix2F mat)
{
  float ox,oy;
  if (!out)return;
...
void gfc_matrix4f_multiply_v(GFC_Vector4DF * out,GFC_Matrix4F mat,GFC_Vector4DF vec)
{
  float ox,oy,oz,ow;
  if (!out)return;
...
void gfc_matrix3f_multiply_v(GFC_Vector3DF * out,GFC_Matrix3F mat,GFC_Vector3DF vec)
{
  float ox,oy,oz;
  if (!out)return;
...
void gfc_matrix2f_multiply_v(GFC_Vector2DF * out,GFC_Matrix2F mat,GFC_Vector2DF vec)
{
  float ox,oy;
  if (!out)return;
...
void gfc_matrix4f_zero(GFC_Matrix4F zero)
{
    memset(zero,0,sizeof(GFC_Matrix4F));
}

void gfc_matrix3f_zero(GFC_Matrix3F zero)
{
    memset(zero,0,sizeof(GFC_Matrix3F));
}

void gfc_matrix2f_zero(GFC_Matrix2F zero)
{
    memset(zero,0,sizeof(GFC_Matrix2F));
}


void gfc_matrix4f_identity(GFC_Matrix4F one)
{
    gfc_matrix4f_zero(one);
    one[0][0] = 1;
    one[1][1] = 1;
    one[2][2] = 1;
    one[3][3] = 1;
...
void gfc_matrix3f_identity(GFC_Matrix3F one)
{
    gfc_matrix3f_zero(one);
    one[0][0] = 1;
    one[1][1] = 1;
    one[2][2] = 1;
}

void gfc_matrix2f_identity(GFC_Matrix2F one)
{
    gfc_matrix2f_zero(one);
    one[0][0] = 1;
    one[1][1] = 1;
}

void gfc_matrix4f_rotate_by_vector(
    GFC_Matrix4F     out,
    GFC_Matrix4F     m,
    GFC_Vector3DF    v
)
{
    gfc_matrix4f_rotate_y(out,m,v.y);
    gfc_matrix4f_rotate_x(out,out,v.x);
    gfc_matrix4f_rotate_z(out,out,v.z);
}

void gfc_matrix4f_rotate_x(
    GFC_Matrix4F     out,
    GFC_Matrix4F     m,
    float       theta
)
{
    ...
    gfc_matrix4f_identity(Result);
    Result[0][0] = cos(-theta);
    Result[0][2] = sin(-theta);
    Result[2][0] = -sin(-theta);
    Result[2][2] = cos(-theta);
    gfc_matrix4f_multiply(out,m,Result);
}

void gfc_matrix4f_rotate_y(
    GFC_Matrix4F     out,
    GFC_Matrix4F     m,
    float       theta
)
...
{
    gfc_matrix4f_from_quaternion(out,gfc_vector4df(axis.x,axis.y,axis.z,radians));
    gfc_matrix4f_multiply(out,m,out);
}

void gfc_matrix4f_perspective(
    GFC_Matrix4F     out,
    float      fov,
    float      aspect,
    float      near,
...
void gfc_matrix4f_view(
    GFC_Matrix4F  out,
    GFC_Vector3DF position,
    GFC_Vector3DF target,
    GFC_Vector3DF up
)
{
    GFC_Vector3DF f,r,u;
    gfc_vector3d_sub(f,position,target);
    gfc_vector3df_normalize(&f);
    
    gfc_vector3df_cross_product(&r,up,f);
    gfc_vector3df_normalize(&r);
    
    gfc_vector3df_cross_product(&u,f,r);
    gfc_vector3df_normalize(&u);
...
void gfc_matrix4f_scale(GFC_Matrix4F out,GFC_Matrix4F in,GFC_Vector3DF scale)
{
    GFC_Matrix4F m;
    gfc_matrix4f_zero(m);
    m[0][0] = scale.x;
...
void gfc_matrix4f_make_translation(
    GFC_Matrix4F out,
    GFC_Vector3DF move
)
{
...
void gfc_matrix3f_make_translation(
    GFC_Matrix3F out,
    GFC_Vector2DF move
)
{
    gfc_matrix3f_identity(out);
    out[2][0] = move.x;
    out[2][1] = move.y;
}

void gfc_matrix4f_translate(GFC_Matrix4F out,GFC_Matrix4F in, GFC_Vector3DF move)
{
    GFC_Matrix4F translate;
    gfc_matrix4f_make_translation(translate,move);
    gfc_matrix4f_multiply(out,in,translate);
}

void gfc_matrix3f_translate(GFC_Matrix3F out,GFC_Matrix3F in, GFC_Vector2DF move)
{
    GFC_Matrix3F translate;
    gfc_matrix3f_make_translation(translate,move);
    gfc_matrix3f_multiply(out,in,translate);
}


void gfc_matrix4f_from_vectors_q(
    GFC_Matrix4F out,
    GFC_Vector3DF translation,
    GFC_Vector4DF quaternion,
    GFC_Vector3DF scale
)
{
    GFC_Matrix4F rotateM;
    ...
    gfc_matrix4f_from_quaternion(rotateM,quaternion);
    gfc_matrix4f_multiply(
            out,
            out,
            rotateM
...
void gfc_matrix4f_from_quaternion(
    GFC_Matrix4F     out,
    GFC_Vector4DF    q)
{
...
void gfc_matrix4_to_matrix16(double m16[16],GFC_Matrix4 m4)
{
    m16[0]  = m4[0][0];
    m16[1]  = m4[0][1];
    m16[2]  = m4[0][2];
...
void gfc_matrix16_to_matrix4(GFC_Matrix4 m4,double m16[16])
{
    m4[0][0] = m16[0]  ;
    m4[0][1] = m16[1]  ;
    m4[0][2] = m16[2]  ;
...
Uint8 gfc_matrix16_invert(double m[16], double invOut[16])
{
    double inv[16], det;
    int i;
...
void gfc_matrix4_multiply_scalar(GFC_Matrix4 out,GFC_Matrix4 m1,double s)
{
    GFC_Matrix4 temp;
    temp[0][0] = s*m1[0][0];
    temp[0][1] = s*m1[0][1];
...
void gfc_matrix3_multiply_scalar(GFC_Matrix3 out,GFC_Matrix3 m1,double s)
{
    GFC_Matrix3 temp;
    temp[0][0] = s*m1[0][0];
    temp[0][1] = s*m1[0][1];
...
void gfc_matrix2_multiply_scalar(GFC_Matrix2 out,GFC_Matrix2 m1,double s)
{
    GFC_Matrix2 temp;
    temp[0][0] = s*m1[0][0];
    temp[0][1] = s*m1[0][1];
